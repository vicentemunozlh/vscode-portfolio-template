  ...
  const highlightedContent = content
    ...
    .map((line) => {
      ...
      if (line.includes('[') && line.includes('](') && line.includes(')')) {
        return line.replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<span class="string">[$1]($2)</span>');
      }
...
export default function CodeEditor({ selectedFile, onCloseFile }: CodeEditorProps) {
  if (!selectedFile) {
    return (
        ...
        <div className="editor-content">
          <div style={{ textAlign: 'center', paddingTop: '100px', color: 'var(--text-muted)' }}>
            ...
            <p style={{ fontSize: '14px', marginTop: '20px' }}>
              Click on any file in the explorer to see example content and structure.
            </p>
            <div style={{ marginTop: '30px', padding: '20px', background: 'var(--sidebar-bg)', borderRadius: '8px', maxWidth: '500px', margin: '30px auto 0' }}>
              <h3 style={{ color: 'var(--accent)', marginBottom: '15px' }}>🚀 Getting Started</h3>
              <ul style={{ textAlign: 'left', lineHeight: '1.8' }}>
                <li>Replace <code>[YOUR NAME]</code> with your actual name</li>
